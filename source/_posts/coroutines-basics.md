...
-   코루틴은 **루틴**의 일종
-   협동 루틴이라 할 수 있으며, Coroutine의 `Co`는 _with_ 또는 _together_를 뜻한다.

<!-- more -->
> 루틴과 코루틴의 차이

![](https://user-images.githubusercontent.com/39554623/120277451-08783500-c2ef-11eb-96e7-403bf6e5d5bd.png)

코루틴은 이전에 자신의 실행이 마지막으로 **중단**되었던 지점 다음의 장소에서 실행을 **재개**한다. (함수의 진입점과 출구점이 여러 개 존재한다)

### [](#코루틴의-역사)코루틴의 역사

코루틴은 멜빈 콘웨이에 의해 1958년 처음 만들어진 용어이며, 이를 어셈블리 프로그램에 적용했다. 코루틴은 협력 작업 cooperative tasks, 예외 exceptions, 이벤트 루프 event loops, 반복자  iterators, 무한 리스트 infinite lists 및 파이프 pipes와 같은 친숙한 프로그램 구성 요소를 구현하는 데 적합하다고 소개되어있는데 [1](#f1) 구글 문서에서는 코루틴의 사용 범위를 좀 더 명확하게 제시하고 있다.

### [](#android-developers-코틀린-가이드에서의-코루틴-정의-sup-id-a12sup)Android Developers 코틀린 가이드에서의 코루틴 정의 [2](#f1)
...
### [](#google-codelabs의-코루틴-정의-sup-id-a13sup)Google Codelabs의 코루틴 정의 [3](#f1)
...
> ### [](#2-your-first-coroutine)② Your first Coroutine
...
> ### [](#3-bridging-blocking-and-non-blocking-worlds)③ Bridging blocking and non-blocking worlds

`Thread.sleep()`과 `delay()`가 혼용되는 것을 막기 위해 `delay()`로 통일해보자. 하지만 CoroutineScope 외부에 `delay()`를 추가하려는데 에러가 발생한다.

![](https://user-images.githubusercontent.com/39554623/120302445-decc0780-c308-11eb-97ae-ede496319166.png)
...
> ### [](#4-rewritten-in-a-more-idiomatic-waysmall관용적인-형태로-만들기small)④ rewritten in a more idiomatic way(관용적인 형태로 만들기)
...
> ### [](#5-waiting-for-a-job)⑤ Waiting for a job
...
> ### [](#7-extract-function-refactoring)⑦ Extract function refactoring
...
![](https://user-images.githubusercontent.com/39554623/120296667-5139e900-c303-11eb-8cac-7727a7e86643.png)

_Suspend function ‘delay’ should be called only from a coroutine or another suspend function_. 즉, 일시 중단되는 함수인 `delay()`는 오직 코루틴이나 다른 suspend function으로부터만 호출될 수 있다는 뜻이다.
...
> ### [](#8-coroutines-are-light-weight)⑧ Coroutines ARE light-weight
...
![](https://user-images.githubusercontent.com/39554623/120319461-ffea2380-c31b-11eb-9d19-0c86475f0b5b.png)
...
![](https://user-images.githubusercontent.com/39554623/120319256-c9140d80-c31b-11eb-9f38-b9e891fd4dcf.png)

이 두 코드를 통해 **코루틴이 스레드보다 구조적으로 상당히 가볍다**는 것을 확인할 수 있다. 많은 스레드를 만들면 부하가 일어날 수 있지만, 코루틴은 많은 양을 생성하여도 크게 부하가 가지 않는 것이다.

> ### [](#9-global-coroutines-are-like-daemon-threads)⑨ Global coroutines are like daemon threads
...
> ### [](#10-suspend-resume)⑩ suspend <-> resume
...
![](https://user-images.githubusercontent.com/39554623/120322472-6886cf80-c31f-11eb-861b-dab8a29df423.png)

![](https://user-images.githubusercontent.com/39554623/120322546-7b010900-c31f-11eb-92e6-54504ccd150f.png)
...
_Coroutine A_가 먼저 출력될 것 같은데 _Coroutine Outer_가 먼저 출력되는 것을 통해 `launch { }`가 코루틴이 실행되기 위한 스케줄링을 해놓는다는 것을 추측할 수 있다.

## [](#정리)정리

코루틴은 **일시 중단이 가능한 계산의 인스턴스**다(an instance of suspendable computation). 다른 코드와 동시에 작동하는 코드 블록을 실행해야 한다는 점에서 스레드와 개념적으로 유사하다. 하지만 코루틴은 특정 스레드에 바인딩되지 않는다. 한 스레드에서 실행을 일시 중지하고 다른 스레드에서 다시 시작할 수 있다.
...
#### [](#launch-main-thread-를-unblocking-한-채-구문-내-작업을-수행)`launch {}` : Main Thread 를 Unblocking 한 채 `{}` 구문 내 작업을 수행

**launch**는 **코루틴 빌더 coroutine builder** 이다. 코루틴 빌더는 새로운 코루틴을 독립적으로 작동하는 나머지 코드들과 **동시에** 실행시킨다. 이것이 _Hello_가 가장 먼저 출력되는 이유이다.
...
#### [](#runblocking-main-thread-를-blocking-한-채-구문-내-작업을-새-thread-에-할당하여-수행)`runBlocking {}` : Main Thread 를 Blocking 한 채 `{}` 구문 내 작업을 새 Thread 에 할당하여 수행
...
**runBlocking**의 이름은 `runBlocking {}` 내부의 모든 코루틴이 실행을 완료할 때까지 이를 실행하는 스레드(여기선 메인 스레드)가 호출되는 동안 Blocking 된다는 것을 의미한다. 스레드는 값 비싼 리소스이고 스레드를 Blocking하는 것은 비효율적이면서 프로그래머가 종종 원하지 않기 때문에 애플리케이션의 최상위 수준에서 실제 코드 내에서는 거의 사용되지 않는 **runBlocking**을 자주 볼 수 있을 것이다.
...
### [](#함수를-추출하여-리팩토링하기)함수를 추출하여 리팩토링하기

`launch {...}` 내부의 코드 블록을 별도의 함수로 추출해보자. 이 코드에 IDE의 _Extract Function_ 기능을 수행하면 아래와 같이 `suspend` 변경자 modifier가 있는 새 함수가 생성이 된다. 일시 중단 함수 suspending function는 일반 함수와 마찬가지로 코루틴 내부에서 사용할 수 있지만 추가 기능은 다른 일시 중단 함수(`delay()`와 같은)를 사용하여 코루틴의 실행을 중단시킬 수 있다는 것이다.

![](https://user-images.githubusercontent.com/39554623/120298097-ba6e2c00-c304-11eb-9271-b3e1f9efe246.png)
...
추출한 함수의 `suspend` 변경자를 제거하면 _Suspend function ‘delay’ should be called only from a coroutine or another suspend function_라는 경고문이 뜬다. 즉, 일시 중단되는 함수인 `delay()`는 오직 코루틴이나 다른 일시 중단 함수로부터만 호출될 수 있다는 뜻이다.

## [](#references)References

-   Kotlin Docs, Coroutines basics : [https://kotlinlang.org/docs/coroutines-basics.html](https://kotlinlang.org/docs/coroutines-basics.html)
-   새차원, 코틀린 코루틴 : [https://youtu.be/14AGUuh8Bp8](https://youtu.be/14AGUuh8Bp8)

* * *

**1** [https://en.wikipedia.org/wiki/Coroutine](https://en.wikipedia.org/wiki/Coroutine) [↩](#a1)

**2** [https://developer.android.com/kotlin/coroutines?hl=ko](https://developer.android.com/kotlin/coroutines?hl=ko) [↩](#a1)

**3** [https://developer.android.com/codelabs/kotlin-coroutines#0](https://developer.android.com/codelabs/kotlin-coroutines#0) [↩](#a1)

#[Kotlin](/tags/Kotlin/)[Coroutine](/tags/Coroutine/)
