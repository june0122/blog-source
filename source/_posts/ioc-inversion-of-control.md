---
title: "제어의 역전(Inversion of Control, IoC) 파헤치기"
date: 2022-07-06T01:29:23.000Z
categories:
  - Design-Pattern
tags:
  - Kotlin
...
**_Inversion of Control(IoC)_**, _제어의 역전_이라고도 번역되는 이 개념은 Dependency Injection(DI)에 대한 개념을 찾아보면 반드시 등장하는 개념입니다. 처음 접했을 땐 여러 블로그의 설명을 읽어도 확실하게 이해가 되지 않아서 사전적인 정의와 예시 코드 정도만 정리하고 넘어갔었는데 안드로이드의 DI 라이브러리인 Hilt에 대해 복습을 하던 중 IoC를 간단명료하게 설명해주는 [유튜브 영상](https://youtu.be/vFzP2SaMyA0)을 발견하여 스스로 IoC에 대해 다시 한 번 정리하고자 글을 작성합니다.

![](https://user-images.githubusercontent.com/39554623/177264393-dbca8fc4-6a65-4e24-a2e5-bd4142f3de76.png)
...
![](https://user-images.githubusercontent.com/39554623/177293086-d1fbfb8b-8c53-4395-8eea-87ff037cfa12.png)

게임 엔진은 라이브러리가 아니라 프레임워크로 분류하는 것이 더 적절합니다. 그리고 프레임워크는 **_inversion of control_**이라는 중요한 개념을 가지고 있습니다.
...
참고로 Inversion of control은 헐리우드(Hollywood) 원칙이라고도 합니다. "우리에게 전화하지 마세요. 우리가 당신을 부를 겁니다(Do not call us, we call You)."

## 반론 댓글

그런데 해당 영상의 댓글에 IoC의 본래 목적에 대해 충분히 설명하고 있지 않다는 의견들이 보였습니다. 한 댓글은 IoC는 일반적인 선형 의존 구조를 따르는 대신 공유 추상화(shared abstraction)를 통해 의존성을 처리하는 아이디어라고 요약하고 있는데 [위키피디아의 IoC 설명](https://en.wikipedia.org/wiki/Inversion_of_control)에서도 유사한 설명을 하고 있습니다.

![](https://user-images.githubusercontent.com/39554623/177344747-fd85bc9d-66e8-42c0-b8e1-dd46a54b49b2.png)
...
![](https://user-images.githubusercontent.com/39554623/177280846-7239dfc6-651c-4874-9a13-9fed3c4af257.png)

### Thái An Lê의 댓글 번역

이 비디오가 내가 알고 있는 Inversion of Control과 동일한 개념을 설명하는지 확실하지 않습니다. 만약 내 코드가 라이브러리를 사용하는 경우에 내 코드가 라이브러리를 호출하는 것은 지극히 정상이며 그 반대는 아닙니다(즉, 라이브러리가 내 코드를 호출). 내가 이해하는 IoC는 **라이브러리가 내 코드에서 사용되는 방식**에 대한 것입니다.

여기서 말하는 제어(control)는 내 코드가 사용하는 이 라이브러리의 구체적인 구현에 대한 제어입니다.
...
'더 많은 통제'는 '더 많은 책임'을 의미하고 이는 '단일 책임 원칙(Single Responsibility Principle, SRP)의 위반'을 뜻합니다.
...
이럴 때 inversion of control이 작동합니다. 이 제어(어떤 구현을 사용할지에 대한 결정)를 시스템의 다른 부분에 양도하고 싶습니다. 내 코드는 오직 라이브러리가 필요한지에만 관심이 있으며 구체적인 구현이 무엇인지는 중요하지 않습니다(즉, 라이브러리가 `LibraryV1`인지 `LibraryV2`인지에 대한 여부). 이것은 **Dependency Injection**을 통해 수행되며 코드는 다음과 같습니다.
...
위의 코드는 Room 라이브러리와 Repository 패턴을 적용한 안드로이드 코드는 IoC를 적용하여 `user`에 대한 제어권을 UserDao 객체에게 이양하고 있습니다. 미리 UserRepositoryImpl 클래스 내부에 user에 대한 행위를 미리 구현해놓으면 UserRepositoryImpl 클래스는 UserDao와 결합하게 되므로 IoC의 목적인 작업의 구현과 수행의 분리(decoupling)에서 벗어나게 됩니다.

## 정리

IoC 란 코드의 흐름을 제어하는 주체가 바뀌는 것입니다.

전통적인 절차적 프로그래밍에선 자신의 코드가 직접 사용할 오브젝트를 결정하고, 결정한 오브젝트를 생성하고, 생성한 오브젝트의 메서드를 호출하는 식이지만 IoC를 적용하면 모든 제어 권한을 자신이 아닌 다른 대상에게 위임합니다. 이를 통해 작업의 구현과 수행을 분리(decoupling)할 수 있고, 코드에 변경사항이 생길 때 부수 효과를 방지할 수 있습니다.

## References

-   Udacity Youtube: [https://youtu.be/vFzP2SaMyA0](https://youtu.be/vFzP2SaMyA0)
-   Stackoverflow: [What is Inversion of Control?](https://stackoverflow.com/a/3140/12364882)
-   Wikipedia: [https://en.wikipedia.org/wiki/Inversion\_of\_control](https://en.wikipedia.org/wiki/Inversion_of_control)
-   [https://johngrib.github.io/wiki/inversion-of-control/](https://johngrib.github.io/wiki/inversion-of-control/)
