---
title: "[Kotlin] Scope function (let, run, with, apply, also) 정리"
date: 2022-03-02T22:01:04.000Z
categories:
  - Kotlin
tags:
  - Kotlin
...
1.  컨텍스트 객체(Context Object)를 블록 내에서 참조하는 방법 : `this` 또는 `it`
2.  반환 값(Return Value) : `Context Object` 또는 `Lambda Result`
...
_※ 인자(parameter)는 함수 정의에 사용되는 변수, 인수(argument)는 함수 호출에 사용되는 변수_
...
![](https://user-images.githubusercontent.com/39554623/156332470-99c38029-5242-48ae-b9d1-427d40c48529.png)

### Return Value

-   `apply`, `also는` context object를 반환
-   `let`, `run`, `with`는 lambda result(람다식의 결과)를 반환
...
#### 2\. 프로퍼티와 함수가 값을 계산하는데 사용되는 도우미 객체(helper object)를 제공할 때
...
-   수신 객체(receiver object)에서 `run`을 호출하는 것이 아닌, 비확장 함수 `run`도 사용할 수 있다.
...
-   외부 범위로부터 `this` 참조를 가리지(shadow) 않길 원할 때
...
Scope function들 외에도 표준 라이브러리는 `takeIf` 와 `takeUnless` 함수를 포함하고 있다. 이 함수들은 연쇄 호출(call chains)에 객체의 상태를 확인하는 작업을 추가하는데, 즉 단일 객체에 대한 필터링 함수이다.

-   `takeIf`는 객체가 술부(predicate)와 일치하는 경우 이 객체를 반환하고, 그렇지 않으면 `null`을 반환한다.
...
두 함수는 `null`을 반환할 수 있기 때문에 뒤에 다른 함수를 연결할 때는 null check를 하거나 safe call(`?.`)을 사용해야 한다.
...
두 함수는 scope function과 함께 사용할 때 유용하다. 좋은 경우는 주어진 술부와 일치하는 객체에서 코드 블럭을 실행하기 위해 `let`을 사용하여 연결하는 것이다. 이렇게 하려면 객체에 대해 `takeIf`를 호출한 다음 safe call(`?.`)로 `let`을 호출한다. 술부와 일치하지 않는 객체의 경우 `takeIf`는 `null`을 반환하고 `let`은 호출되지 않는다.
...
-   Kotlin Docs: [https://kotlinlang.org/docs/scope-functions.html](https://kotlinlang.org/docs/scope-functions.html)
