---
title: "[Android] Fragment간 데이터 전달"
date: 2021-05-25T23:45:13.980Z
categories:
  - Android
  - Guide
tags:
...
<!-- more -->
## [](#단일-액티비티-프래그먼트의-우두머리)단일 액티비티: 프래그먼트의 우두머리
...
즉, 위의 코드에서 CrimeListFragment는 CrimeFragment를 MainActivity에 추가하면서 MainActivity의 레이아웃에 fragment\_container가 있을 것이라고 가정한다. 그러나 이런 일은 CrimeListFragment의 호스팅 액티비티인 MainActivity가 해야 할 일이다.

따라서 여기서는 프래그먼트의 독립성을 유지하기 위해 프래그먼트에 콜백 인터페이스를 정의하고 호스팅 액티비티가 해당 콜백 인터페이스를 구현해 프래그먼트 교체를 수행하게 한다.

### [](#프래그먼트-콜백-인터페이스)프래그먼트 콜백 인터페이스
...
![](https://user-images.githubusercontent.com/39554623/119394546-4385d180-bd0d-11eb-98e5-9702783fca0b.gif)
...
프래그먼트 인자 fragment argument를 사용하면 프래그먼트에 속하는 어딘가에 데이터를 저장할 수 있다. 여기서 프래그먼트에 속하는 _‘어딘가’_ 는 **인자 번들 argument bundle** 을 말한다. 프래그먼트는 자신의 부모 액티비티나 다른 외부 소스에 의존하지 않고 인자 번들로부터 데이터를 가져올 수 있다.
...
### [](#인자를-프래그먼트에-첨부하기)인자를 프래그먼트에 첨부하기
...
## [](#상세-내역-화면에-보여줄-crime-객체를-livedata-변환으로-얻기)상세 내역 화면에 보여줄 Crime 객체를 LiveData 변환으로 얻기
...
![](https://user-images.githubusercontent.com/39554623/119422693-91fe9480-bd3c-11eb-8809-1e86f2be6e21.gif)

#### [](#애니메이션-깜빡거림-없애기)애니메이션 깜빡거림 없애기
...
## [](#데이터베이스-변경하기)데이터베이스 변경하기

사용자가 상세 내역 화면을 벗어날 때 사용자가 변경한 데이터를 데이터베이스에 저장해야 한다.

우선 기존의 범죄 데이터를 변경하는 함수와 새로운 데이터를 추가하는 함수를 **CrimeDao**에 추가한다. 단, 새 데이터 추가 함수는 링크 추가 예정에서 다룬다.
...
### [](#프래그먼트-생명주기에-맞춰-데이터베이스에-데이터-쓰기)프래그먼트 생명주기에 맞춰 데이터베이스에 데이터 쓰기

마지막으로 사용자가 상세 내역 화면에서 입력한 데이터를 데이터베이스에 쓰도록 변경한다. 이 작업은 사용자가 상세 내역 화면을 벗어날 때 수행한다.

> 데이터베이스에 변경하기 ([CrimeDetailViewModel.ky](http://CrimeDetailViewModel.ky))
...
![](https://user-images.githubusercontent.com/39554623/119434140-861dcd00-bd52-11eb-8011-bebaa074fd7e.gif)

* * *

## [](#️-궁금증-프래그먼트-인자를-사용하는-이유는)`💁🏻‍♂️ 궁금증` 프래그먼트 인자를 사용하는 이유는?
...
## [](#그외의-fragment간-데이터-전달-방법들)그외의 Fragment간 데이터 전달 방법들
...
## [](#챌린지-diffutil-효율적으로-recyclerview-다시-로드하기)`👨🏻‍💻 챌린지` [**DiffUtil**](https://developer.android.com/reference/androidx/recyclerview/widget/DiffUtil) : 효율적으로 RecyclerView 다시 로드하기
...
#[Android](/tags/Android/)[Kotlin](/tags/Kotlin/)[RecyclerView](/tags/RecyclerView/)[Database](/tags/Database/)[Room](/tags/Room/)[Fragment](/tags/Fragment/)[DiffUtil](/tags/DiffUtil/)[Executors](/tags/Executors/)
