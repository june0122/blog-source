---
title: "[Android] Dialog - 대화상자"
date: 2021-05-28T01:20:19.575Z
categories:
  - Android
  - Guide
tags:
...
<!-- more -->
![](https://user-images.githubusercontent.com/39554623/119682086-91214c00-be7d-11eb-9907-b8a5cd9c29b1.png)
...
## [](#dialogfragment-생성하기)DialogFragment 생성하기
...
![](https://user-images.githubusercontent.com/39554623/119649087-337d0780-be5d-11eb-8c3f-58d1428e3df0.jpeg)
...
### [](#dialogfragment-보여주기)DialogFragment 보여주기
...
`this@CrimeFragment`는 DatePickerFragment가 아닌 CrimeFragment로부터 `requireFragmentManager()`를 호출하기 위해 필요하다. 여기서는 _apply_ 블록 내부의 `this`가 DatePickerFragment를 참조하므로 this 다음에 `@CrimeFragment`를 지정했다.

> _apply_ 블록 내부의 `this`가 DatePickerFragment를 참조

![](https://user-images.githubusercontent.com/39554623/119652122-ae93ed00-be60-11eb-98f7-f6999424dfe5.png)
...
![](https://user-images.githubusercontent.com/39554623/119654067-029fd100-be63-11eb-83fd-4c7cbb42e986.png)

## [](#두-프래그먼트-간의-데이터-전달하기)두 프래그먼트 간의 데이터 전달하기
...
![](https://user-images.githubusercontent.com/39554623/119649085-324bda80-be5d-11eb-9a3b-c4f164105e45.jpeg)
...
![](https://user-images.githubusercontent.com/39554623/119649074-30821700-be5d-11eb-8072-2063c4718276.jpeg)

### [](#datepickerfragment에-데이터-전달하기)DatePickerFragment에 데이터 전달하기
...
여기서 `arguments`는 DatePickerFragment의 속성 (최상위 슈퍼 클래스인 Fragment로부터 DialogFragment로 상속되고 다시 DatePickerFragment로 상속됨)이며, 프래그먼트 인자를 갖는다.
...
### [](#crimefragment로-데이터-반환하기)CrimeFragment로 데이터 반환하기
...
#### [](#대상-프래그먼트로-데이터-전달하기)대상 프래그먼트로 데이터 전달하기
...
OnDateSetListener는 사용자가 선택한 날짜를 받는데 사용된다. 첫 번째 매개변수는 결과가 산출되는 DatePicker 객체이며, 여기서는 사용하지 않으므로 밑줄 `_`을 지정하였다.(코틀린에서 사용되지 않은 매개변수를 나타낼 때 밑줄을 사용)
...
`targetFragment` 속성은 DatePickerFragment와 연관된 프래그먼트(여기선 CrimeFragment) 인스턴스 참조를 갖는다. 이 속성은 null 값을 가질 수 있으므로 _null에 안전한 let 블록이 사용되었다._ let 블록에서는 `targetFragment` 속성이 참조하는 프래그먼트 인스턴스의 타입을 Callbacks 인터페이스 타입으로 변환한 후, 새로운 날짜를 인자로 전달해 `onDateSelected()` 함수를 호출한다. 따라서 CrimeFragment에 구현된 `onDateSelected()` 함수가 호출되어 실행되므로, 사용자가 선택한 날자가 CrimeFragment에 전달될 수 있다.

#[Android](/tags/Android/)[Kotlin](/tags/Kotlin/)[Dialog](/tags/Dialog/)
