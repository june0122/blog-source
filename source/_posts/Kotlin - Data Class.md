...
## [](#1-모든-클래스가-정의해야-하는-메소드)1\. 모든 클래스가 정의해야 하는 메소드
...
### [](#1-1-문자열-표현-tostring)1-1. 문자열 표현 : `toString()`
...
### [](#1-2-객체의-동등성-equals)1-2. 객체의 동등성 : `equals()`
...
-   참고로 코틀린에서 `==` 연산자는 **참조 동일성을 검사하지 않고 객체의 동등성을 검사**한다. 따라서 `==` 연산은 `equals`를 호출하는 식으로 컴파일된다.[1](#f1)
...
### [](#1-3-해시-컨테이너-hashcode)1-3. 해시 컨테이너: `hashCode()`
...
## [](#2-데이터-클래스-모든-클래스가-정의해야-하는-메소드-자동-생성)2\. 데이터 클래스: 모든 클래스가 정의해야 하는 메소드 자동 생성
...
### [](#2-1-데이터-클래스와-불변성-copy-메소드)2-1. 데이터 클래스와 불변성: `copy()` 메소드
...
### [](#2-2-구조-분해-선언과-component-함수)2-2. 구조 분해 선언과 component 함수

구조 분해 선언 destructuring declaration이라는 개념에서 **구조 분해를 사용하면 복합적인 값을 분해해서 여러 다른 변수를 한꺼번에 초기화 할 수 있다.**
...
-   [Kotlin in Action](http://www.acornpub.co.kr/book/kotlin-in-action#toc) - 4.3절 171p, 7.4절 326p
-   [https://kotlinlang.org/docs/data-classes.html#data-classes-and-destructuring-declarations](https://kotlinlang.org/docs/data-classes.html#data-classes-and-destructuring-declarations)
-   [https://kotlinlang.org/docs/destructuring-declarations.html#example-returning-two-values-from-a-function](https://kotlinlang.org/docs/destructuring-declarations.html#example-returning-two-values-from-a-function)
...
자바에서는 `==`를 원시 타입과 참조 타입을 비교할 때 사용한다. 원시 타입의 경우 `==`는 두 피연사자의 값이 같은지 비교한다 동등성(equality). 반면 참조 타입의 경우 `==`는 두 피연산자의 주소가 같은지를 비교한다 참조 비교(reference comparision). 따라서 자바에서는 두 객체의 동등성을 알려면 `equals`를 호출해야 한다. 자바에서는 `equals` 대신 `==`를 호출하면 문제가 될 수 있다는 사실도 아주 잘 알려져 있다.
